...
/** @type {string[]} */
const paths = result.files.map(/** @param {{ path: string }} f */ (f) => f.path);
/** @param {string} p */
const has = (p) => paths.includes(p);
/** @param {string} prefix */
const hasPrefix = (prefix) => paths.some(/** @param {string} p */ (p) => p.startsWith(prefix));
